...
genfSVM.PCA <- function(N, K, bfun, lambdas, grids, eigenfunction, noise.sigma){
...
data_fsvm <- function(Ntrain, K, bfun, lambdas, grids, eigenfunction,noise.sigma){
  ...
  train = genfSVM.PCA(Ntrain,K, bfun,lambdas,grids,eigenfunction,noise.sigma)
...
for (sim in sims){
  ...
  for (l in 1:length(noise.sigmas)){
    ...
    Ntest = 10000
    set.seed(2)
    test = genfSVM.PCA(Ntest, K, bfun,lambdas,grids, eigenfunction,noise.sigma)
